...
import CodeInputField from "./CodeInput.js";
...
describe("CodeInputField", () => {
  ...
  test(`renders component with value: ${numbers}`, () => {
    const wrapper = render(
        <CodeInputField value={numbers} fields={6} type="number" />,
      ),
      val = [];
  ...
  test(`renders component with value: ${chars}`, () => {
    const wrapper = render(
        <CodeInputField value={chars} fields={6} type="text" />,
      ),
      val = [];
  ...
  test("simulates onKeydown events; start at 2nd input", () => {
    ...
    expect(wrapper.find("input").at(0).instance()).toEqual(
  ...
  test("simulates onChange from paste type=text", () => {
    const onChange = jest.fn();
    const wrapper = mount(
      <CodeInputField onChange={onChange} fields={6} type="text" />,
    );
    const element = wrapper.find("input").at(0);
  ...
  test("simulates onChange from paste type=number", () => {
    const onChange = jest.fn();
    const wrapper = mount(
      <CodeInputField onChange={onChange} fields={6} type="number" />,
    );
    const element = wrapper.find("input").at(0);
    const target = element.instance();
  ...
  test("focuses next on right arrow key press", () => {
    ...
    expect(wrapper.find("input").at(2).instance()).toEqual(
      document.activeElement,
    );
  });
  ...
  test("focuses previous on left arrow key press", () => {
    ...
    expect(wrapper.find("input").at(1).instance()).toEqual(
      document.activeElement,
    );
  });
  ...
  test("on down arrow key press", () => {
    const wrapper = mount(
      <CodeInputField fields={4} value="123" type="number" />,
    );
    const element = wrapper.find("input").at(0);
  ...
  test("should be disabled", () => {
    const wrapper = mount(
      <CodeInputField fields={4} value="123" type="number" disabled />,
    );
  ...
  test("should only use uppercase letters", () => {
    const wrapper = mount(
      <CodeInputField fields={3} type="text" forceUppercase />,
    );
    const element = wrapper.find("input").at(0);
    const target = element.instance();
    target.value = "abc";
